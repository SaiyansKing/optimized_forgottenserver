...
function (pvs_studio_analyze_file SOURCE SOURCE_DIR BINARY_DIR)
    ...
    if (EXISTS "${SOURCE}" AND NOT TARGET "${LOG}" AND NOT "${PVS_STUDIO_LANGUAGE}" STREQUAL "")
        ...
        add_custom_command(OUTPUT "${LOG}"
                           ...
                           DEPENDS "${SOURCE}" "${PVS_STUDIO_SUPPRESS_BASE}"
...
function (pvs_studio_add_target)
    ...
    if (PVS_STUDIO_COMPILE_COMMANDS)
        ...
        add_custom_command(
            ...
            DEPENDS "${PVS_STUDIO_SUPPRESS_BASE}"
        )
...
option(TOOLS_ENABLE_PVS
       "Enable static analysis with pvs-studio"
       FALSE
      )

if(TOOLS_ENABLE_PVS)
    log_option_enabled("pvs-studio")
else()
    log_option_disabled("pvs-studio")
endif(TOOLS_ENABLE_PVS)

function(add_pvs_target TARGET)
    if(TOOLS_ENABLE_PVS)
        set(PVS_TARGET_NAME ${TARGET}_pvs)

        pvs_studio_add_target(TARGET ${PVS_TARGET_NAME} ALL
                            OUTPUT FORMAT errorfile
                            ANALYZE ${TARGET}
                            MODE GA:1,2,3 64:1,2,3 OP:1,2,3 CS:1,2,3
                            LOG ${PVS_TARGET_NAME}.out
                            )
    endif()
endfunction()
